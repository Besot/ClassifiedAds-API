...
namespace AlutaMartAPI.Services.Classes
{
    public class WishlistService(IUnitOfWork _unitOfWork, IResponseService _responseService) : IWishlistService
    {
        ...
        public async Task<ServiceResponse<string>> AddToWishlistAsync(AddToWishlistDTO model, UserDTO currentUser)
        {
            try
            {
                if (currentUser.Id == Guid.Empty) return _responseService.ErrorResponse<string>("Unauthorized request");
                ...
                var wishlist = await _unitOfWork.Context.Wishlists
                    .Include(w => w.WishlistItems)
                    .FirstOrDefaultAsync(w => w.ProfileId == currentUser.Id);
        ...
        public async Task<ServiceResponse<string>> ClearWishlistAsync(UserDTO currentUser)
        {
            try
            {
                if (currentUser.Id == Guid.Empty)return _responseService.ErrorResponse<string>($"You must be logged in as a buyer to manage wishlist");
        ...
        public async Task<ServiceResponse<GetWishlistDTO>> GetWishlistAsync(UserDTO currentUser)
        {
            try
            {
                if (currentUser.Id == Guid.Empty)return _responseService.ErrorResponse<GetWishlistDTO>($"You must be logged in as a buyer to manage wishlist");
        ...
        public async Task<ServiceResponse<string>> RemoveFromWishlistAsync(Guid wishlistItemId, UserDTO currentUser)
        {
            try
            {
                ...
                var wishlistItem = await _unitOfWork.Context.WishlistItems
                    .AsNoTracking()
                ...
                if (wishlistItem.Wishlist.ProfileId != currentUser.Id)return _responseService.ErrorResponse<string>($"You can only remove items from your own wishlist");
